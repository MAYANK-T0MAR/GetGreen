	  ...
	  <path fill-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"/>
					        ...
					        <div id="submenu-2" class="collapse submenu submenu-2 show" data-bs-parent="#menu-accordion">
						        <ul class="submenu-list list-unstyled">
							        ...
							        <li class="submenu-item"><a class="submenu-link active" href="edit-product.php">Edit Product</a></li>
						        </ul>
					        </div>
					    </li><!--//nav-item-->
				    </ul><!--//app-menu-->
...
// Check if the product ID is provided in the URL parameters
if (isset($_GET['product_id'])) {
    $product_id = $_GET['product_id'];

    // Debugging: Check the value of $product_id
    echo "Product ID: " . $product_id;
    // Fetch product details from the 'plants' table based on the provided product ID
    $sql = "SELECT * FROM plants WHERE id = $product_id";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Product found, retrieve its details
        $product = $result->fetch_assoc();
    } else {
        // Product not found, handle the error (redirect or display an error message)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sizeDescription = htmlspecialchars($_POST['sizeDescription']);
    $img = htmlspecialchars($_POST['img']);
    $price = floatval($_POST['price']); // Convert to float for numeric input
    $best = isset($_POST['best']) ? 1 : 0; // Convert checkbox value to boolean
    $featured = isset($_POST['featured']) ? 1 : 0; // Convert checkbox value to boolean
    $description = htmlspecialchars($_POST['description']);
    $light = htmlspecialchars($_POST['light']);
    $care = htmlspecialchars($_POST['care']);
    $product_id = $_POST['product_id']; // Add this line to get the product ID

    // You might want to add additional validation and error handling here

    // SQL update query
    $sql = "UPDATE plants SET name=?, size=?, sizeDescription=?, img=?, price=?, best=?, featured=?, description=?, light=?, care=? WHERE id=?";

    // Prepare the statement
    $stmt = $conn->prepare($sql);

    // Bind parameters
    $stmt->bind_param("ssssdiisssi", $name, $size, $sizeDescription, $img, $price, $best, $featured, $description, $light, $care, $product_id);
                        ...
                        <div class="form-holder">
                            <div class="form-content">
                                <div class="form-items">
                                    <h3>Edit Product</h3>
                                    <p>Fill or Update in the data below.</p>
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="requires-validation" novalidate>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input class="form-control" type="text" id="size" name="size" value="<?php echo $product['size']; ?>" required>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea id="sizeDescription" name="sizeDescription" class="form-control" required><?php echo htmlspecialchars($product['sizeDescription']); ?></textarea>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input class="form-control" type="text" id="img" name="img" value="<?php echo $product['img']; ?>" required>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input class="form-control" type="number" id="price" name="price" min="0" step="0.01" value="<?php echo $product['price']; ?>" required>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <input type="checkbox" id="best" name="best" <?php if ($product['best'] == 1) echo "checked"; ?>>
                                        </div>

                                        <div class="col-md-12">
                                            <label for="featured">Featured:</label>
                                            <input type="checkbox" id="featured" name="featured" <?php if ($product['featured'] == 1) echo "checked"; ?>>
                                        </div>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea id="light" name="light" class="form-control" required><?php echo $product['light']; ?></textarea>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea id="care" name="care" class="form-control" required><?php echo $product['care']; ?></textarea>
                                            <div class="valid-feedback">Care field is valid!</div>
                                            <div class="invalid-feedback">Care field cannot be blank!</div>
                                        </div>
                                    
                                        <div class="form-button mt-3">
                                            <button type="submit" name="insert_Product" class="btn btn-primary">Update Product</button>
                                        </div>
                                    </form>
